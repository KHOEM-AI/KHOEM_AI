# 🚀 KHOEM_AI Global Time & Countries Engine (`khoemai.md`)

> **System Status:** `Active` | **Version:** `3.3 PRO` | **Module:** `Global Time & Timezone Intelligence (195 Nations)`

---

## 📌 1. Executive Overview
The **KHOEM_AI Global Time & Countries** module provides a high-performance world clock and timezone selector covering **all 195 sovereign nations**, allowing users to check real-time local hours across different global regions dynamically.

---

## 🛠️ 2. Backend API Endpoint (`Python / Flask`)
...
def global_time():
    """API endpoint to fetch local time based on specific timezone"""
    timezone_name = request.args.get("tz", "Asia/Phnom_Penh")
    try:
        current_time = datetime.datetime.now(ZoneInfo(timezone_name))
        return jsonify({
            "status": "success",
...
# 💻 3. Frontend Script & Full 195 Countries Dataset (khoemai.js)
...
document.addEventListener("DOMContentLoaded", function() {
    const countries = [
        ...
        { name: "Maldives", native: "ދިވެހިރާއްޖེ་", flag: "🇲🇻", tz: "Indian/Maldives" },
    ...
    const selectEl = document.getElementById("kmLangSelect");
    const timeEl = document.getElementById("kmClockTime");
    ...
    function updateTime() {
        const country = countries[selectEl.value];
        try {
            ...
            timeEl.innerText = formatter.format(new Date());
        } catch (e) {
            timeEl.innerText = "N/A";
        }
    }

    selectEl.addEventListener("change", updateTime);
    setInterval(updateTime, 1000);
    updateTime();
});

# 🌐 4. HTML Structure Integration

<div class="khoemai-container">
    <h3>Select Country & Language</h3>
    <select id="kmLangSelect" class="khoemai-select">
        <option value="">Loading 195 Countries...</option>
    </select>
    <div class="khoemai-clock-display">
        <span>Local Time:</span>
        <span id="kmClockTime">--:--:--</span>
    </div>
</div>

# 🚀 KHOEM_AI — Global Time & Countries Engine

> **Status:** `Active` &nbsp;|&nbsp; **Version:** `3.3 PRO` &nbsp;|&nbsp; **Module:** `Global Time & Timezone Intelligence (195 Nations)`
...
3. [Backend API Endpoint — Python / Flask](#3-backend-api-endpoint--python--flask)
4. [Frontend Script — khoemai.js](#4-frontend-script--khoemaijs)
5. [HTML Template — khoemai.html](#5-html-template--khoemaihtml)
6. [CSS Styles — khoemai.css](#6-css-styles--khoemaicss)
...
The **KHOEM_AI Global Time & Countries** module is a lightweight, zero-dependency world clock and timezone selector covering all **195 sovereign nations**.

**Key features:**

- 🌍 Full 195-country dataset with flag emoji, native name, and IANA timezone
- 🕐 Real-time clock that updates every second using the browser's `Intl.DateTimeFormat` API
- 📍 Auto-detects the user's local timezone on page load and pre-selects the matching country
- ♿ Accessible — uses `<label>`, `aria-label`, and `aria-live` attributes
...
khoem-new/
├── docs/
│   └── khoemai.md          ← This documentation file
├── static/
│   ├── css/
│   │   └── khoemai.css     ← Widget styles
│   └── js/
│       └── khoemai.js      ← Country data + clock logic
└── templates/
    └── khoemai.html        ← HTML template (Flask / Jinja2)
```

---

## 3. Backend API Endpoint — Python / Flask

Optional server-side endpoint for fetching the current time by timezone name.
...
@app.route("/api/global-time", methods=["GET"])
def global_time():
    """Return the current local time for a given IANA timezone."""
    timezone_name = request.args.get("tz", "Asia/Phnom_Penh")
    try:
        current_time = datetime.datetime.now(ZoneInfo(timezone_name))
        return jsonify({
...
|-----------|---------|-------------|
| `tz` | `Asia/Phnom_Penh` | Any valid IANA timezone string (e.g. `Asia/Tokyo`) |

**Example request:**

```
GET /api/global-time?tz=Asia/Tokyo
```
...
```json
{
  "status": "success",
  "timezone": "Asia/Tokyo",
  "time": "2025-08-06 14:35:22"
}
```

---

## 4. Frontend Script — `khoemai.js`

Location: `khoem-new/static/js/khoemai.js`

### Key improvements over the original

| Item | Before | After |
|------|--------|-------|
| Array index | `countries[selectEl.value]` (string coercion) | `countries[parseInt(selectEl.value, 10)]` |
| DOM update | `innerText` | `textContent` (standard) |
| Auto-detect timezone | ❌ | ✅ `Intl.DateTimeFormat().resolvedOptions().timeZone` |
| Interval reference | Not stored | Stored in `intervalId` |
...
DOMContentLoaded
    │
    ├─ Build <option> list from countries array
    │
    ├─ Auto-detect browser timezone → pre-select matching country
    │
    ├─ Call updateTime() immediately
    │
    └─ setInterval(updateTime, 1000)  ← ticks every second
```

### Core functions

```javascript
// ទទួលប្រទេសសកម្មចេញពី <select>
function getSelectedCountry() {
    return countries[parseInt(selectEl.value, 10)];
}

// អាប់ដេតម៉ោងតាមប្រទេសដែលបានជ្រើស
function updateTime() {
...
Each entry in the `countries` array follows this shape:

```javascript
{
    name:   "Cambodia",       // English name
    native: "កម្ពុជា",         // Native language name
    flag:   "🇰🇭",            // Flag emoji (Unicode regional indicators)
    tz:     "Asia/Phnom_Penh" // IANA timezone identifier
}
```

---

## 5. HTML Template — `khoemai.html`

Location: `khoem-new/templates/khoemai.html`
...
<head>
    ...
    <title>KHOEM_AI - Global Time &amp; Countries</title>
    <link rel="stylesheet" href="../static/css/khoemai.css">
</head>
<body>

    <div class="khoemai-container">
        <h3>🌍 Select Country &amp; Timezone</h3>

        <label for="kmLangSelect" class="khoemai-label">Country</label>
        <select id="kmLangSelect" class="khoemai-select"
                aria-label="Select a country to view its local time">
            <option value="">⚡ Loading 195 Countries...</option>
        </select>

        <div class="khoemai-clock-display"
             role="status" aria-live="polite" aria-label="Local time display">
            <span class="khoemai-clock-label">🕐 Local Time:</span>
            <span id="kmClockTime">--:--:-- --</span>
        </div>
    </div>

    <script src="../static/js/khoemai.js"></script>
...
The JavaScript relies on exactly these two IDs:

| ID | Element | Purpose |
|----|---------|---------|
| `kmLangSelect` | `<select>` | Country dropdown — populated by JS |
| `kmClockTime` | `<span>` | Clock display — updated every second |

---

## 6. CSS Styles — `khoemai.css`

Location: `khoem-new/static/css/khoemai.css`

All colours and sizes are controlled via **CSS custom properties** defined in `:root`, making theme changes a single-place edit:

```css
:root {
    --khoemai-bg:           #f8fafc;
    --khoemai-card:         #ffffff;
    --khoemai-border:       #e1e8ed;
    --khoemai-border-focus: #0f172a;
    --khoemai-accent:       #0f172a;
    --khoemai-text:         #1e293b;
    --khoemai-muted:        #64748b;
    --khoemai-clock-bg:     #e2e8f0;
    --khoemai-radius-sm:    8px;
    --khoemai-radius-md:    12px;
    --khoemai-shadow:       0 4px 15px rgba(0, 0, 0, 0.08);
    --khoemai-transition:   all 0.25s ease;
}
```

**To switch to a dark theme**, override the variables:
...
:root {
    --khoemai-bg:       #0f172a;
    --khoemai-card:     #1e293b;
    --khoemai-border:   #334155;
    --khoemai-accent:   #38bdf8;
    --khoemai-text:     #f8fafc;
    --khoemai-muted:    #94a3b8;
    --khoemai-clock-bg: #0f172a;
}
```
...
```
Browser loads khoemai.html
        │
        ├─ Loads khoemai.css  →  styles the widget
        │
        └─ Loads khoemai.js
                │
                ├─ Reads browser timezone (Intl API)
                ├─ Builds <option> list (195 countries)
                ├─ Pre-selects user's country
                ├─ Renders current time immediately
                └─ setInterval → updates clock every 1 second
```

> **Note:** The clock runs entirely in the browser using `Intl.DateTimeFormat`.  
> The Flask `/api/global-time` endpoint is **optional** — it is only needed if you want server-side time fetching (e.g. for bots, APIs, or environments without JavaScript).

---

*KHOEM_AI Global Clock — Documentation v3.3 PRO*
